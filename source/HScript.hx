	...
	{		
		...
		var boolArray:Array<Bool> = [
              ...
              (
                  ...
                  #end
                  openfl.utils.Assets.exists(Paths.getPreloadPath('$scriptPath.$ext'))
              )
        ];
		...
		if (boolArray.contains(true))
		{
			...
			try
			{
				...
				for (scripts in [File.getContent(Paths.modFolders(path)), parser.parseString(openfl.utils.Assets.getText(Paths.getPreloadPath(path)))])
				     expr = scripts;
